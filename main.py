...
def login():
    ...
    if (request.method == 'POST'):
        ...
        if (result):
            ...
            if (who == "Employer"):
                return redirect(url_for('EmployerIndex'))
            # return render_template('EmployerIndex.html', user = current_user_name)

        if (not result and current_user_name == "Invalid Email"):
            return render_template('login.html', message = "Invalid Email ID, try again!!")

        if (not result and current_user_name == "Invalid Password"):
            return render_template('login.html', message = "Password is Incorrect, try again!!")
